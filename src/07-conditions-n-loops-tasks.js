...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < split.length; i += 1) {
    if (split.indexOf(split[i]) === split.lastIndexOf(split[i])) return split[i];
  }
  return null;
}
...
function isCreditCardNumber(ccn) {
  ...
  let v = ccn;
  while (v) {
    nums.unshift(v - Math.floor(v / 10) * 10);
    v = Math.floor(v / 10);
  }
  for (let i = nums.length - 2; i >= 0; i -= 2) {
    nums[i] *= 2;
  }
  return nums.reduce((sum, el, i) => {
    if (i % 2 === nums.length % 2) {
      return sum + (el % 10) + Math.floor(el / 10);
    }
    return sum + el;
  }, 0) % 10 === 0;
}
...
function getCommonDirectoryPath(pathes) {
  let res = '';
  const path = pathes[1].split('');
  for (let i = 0; i < path.length; i += 1) {
    if (pathes.every((el) => el[i] === path[i])) {
      res += path[i];
    } else {
      break;
    }
